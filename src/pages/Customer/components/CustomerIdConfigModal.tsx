...
interface CustomerIdConfigModalProps {
...
const CustomerIdConfigModal: React.FC<CustomerIdConfigModalProps> = ({
  idConfigModalOpen,
          ...
          <DraggableWrapper>
                <div className="bg-white rounded-2xl w-full max-w-[400px] shadow-2xl overflow-hidden border-t-[6px] border-[#ab8a3b]" onClick={e => e.stopPropagation()}>
                  <div className="px-6 py-2.5 border-b border-slate-100 flex justify-between items-center">
                    <h3 className="font-black text-[#111f42] text-[12px] uppercase tracking-widest flex items-center gap-2"><Settings2 size={16} className="text-[#ab8a3b]" /> Config Customer ID</h3>
                  ...
                  <div className="p-4 sm:p-5 space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <input value={idConfig.prefix} onChange={e => setIdConfig({ ...idConfig, prefix: e.target.value.toUpperCase() })} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg outline-none focus:border-[#ab8a3b] font-mono font-bold text-[13px]" />
                    </div>
                    <div className="space-y-1.5">
                    ...
                    <div className="mt-4 p-4 bg-slate-900 rounded-xl border-l-4 border-[#ab8a3b]">
                      ...
                      <p className="text-[14px] font-black text-white font-mono">{generateCustomerID()}</p>
                    </div>
                  </div>
                  <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end">
                    <button onClick={() => { setIdConfigModalOpen(false); setForm({ ...form, customerID: generateCustomerID() }); }} className="bg-[#ab8a3b] text-[#111f42] px-6 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest hover:brightness-95 transition-all">Apply Generator</button>
                  </div>
                </div>
              </DraggableWrapper>
...
export default CustomerIdConfigModal;
