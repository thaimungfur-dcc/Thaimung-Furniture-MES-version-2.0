  ...
  return (
    <div className="bg-white p-5 rounded-none border-2 border-slate-100 shadow-sm flex flex-col justify-between relative overflow-hidden h-[130px] group transition-all hover:border-[#ab8a3b]/30">
      <div className="z-10 flex justify-between items-start">
        <p className="font-black text-[10px] uppercase tracking-[0.2em]" style={{ color }}>{title}</p>
        <div className="w-10 h-10 rounded-none flex items-center justify-center border-2 shadow-inner" style={{ backgroundColor: bgColor, color, borderColor: `${color}20` }}>
          <Icon size={20} />
        </div>
      </div>
      <div className="z-10 mt-auto">
        <h3 className="text-[32px] font-black tracking-tighter leading-none" style={{ color: '#111f42' }}>{value}</h3>
      </div>
