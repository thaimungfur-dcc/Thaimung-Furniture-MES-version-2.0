...
export default function FurnitureCatalogueApp() {
    ...
    return (
        <div className="flex flex-col space-y-4 w-full relative flex-1 animate-fade-in-up">
            ...
            {/* Main Container */}
            <main className="flex flex-col w-full relative z-10 flex-1 gap-2">
                {viewMode === 'dashboard' && (
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mt-2 flex-1 min-h-[400px]">
                       <CatalogueDashboard products={products} />
                    </div>
                )}

                {viewMode !== 'dashboard' && (
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col flex-1 overflow-hidden mt-2 min-h-[500px]">
                        <div className="flex flex-col lg:flex-row items-center justify-between gap-4 p-4 border-b border-slate-100 bg-slate-50/30">
                            <div className="flex flex-1 items-center gap-3 w-full lg:w-auto overflow-x-auto no-scrollbar">
                                ...
                                <div className="relative w-full lg:w-64 shrink-0">
                                    ...
                                    <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} placeholder="Search Catalogue..." className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-[11px] font-bold outline-none focus:border-[#111f42] uppercase tracking-widest" />
                                </div>
                            </div>
                            <div className="flex gap-3 shrink-0 flex-nowrap items-center ml-auto">
                                <button onClick={() => openModal()} className="py-2.5 px-6 rounded-xl text-[11px] font-black bg-[#111f42] text-white shadow-md flex items-center gap-2 uppercase tracking-wide whitespace-nowrap hover:bg-[#1e346b] transition-all"><Plus size={16} className="text-[#ab8a3b]" /> ADD FURNITURE</button>
                            </div>
                        </div>

                        {/* Scrollable Content */}
                        <div className="flex-1 w-full max-h-full overflow-hidden flex flex-col">
                            {viewMode === 'grid' ? (
                                <div className="p-6 flex flex-col flex-1 overflow-y-auto master-custom-scrollbar">
                                    ...
                                    <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 flex-shrink-0">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="px-4 text-[11px] font-bold text-[#111f42] bg-white border border-slate-200 py-2.5 rounded-lg shadow-sm uppercase tracking-widest">PAGE {currentPage} / {totalPages}</span>
                                ...
                                <div className="flex-1 overflow-hidden -mx-px -mb-px">
