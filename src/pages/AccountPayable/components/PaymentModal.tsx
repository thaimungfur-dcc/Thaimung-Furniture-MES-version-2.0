    ...
    <div className="fixed inset-0 bg-[#223149]/40 backdrop-blur-sm z-[60] flex items-center justify-center p-4 no-print">
          ...
          <DraggableWrapper>
                <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
                  <div className="px-6 py-2.5 flex justify-between items-center bg-[#223149] text-white">
                    <h2 className="text-lg font-semibold uppercase tracking-widest flex items-center gap-2">
                      <CreditCard size={20} className="text-[#f2b33d]" /> Record Payment Out
                    </h2>
                  ...
                  <form onSubmit={handlePaymentSubmit} className="p-4 sm:p-5 space-y-5">
                    <div className="bg-gradient-to-br from-[#f0ede5]/50 to-[#f5f0e9] p-4 rounded-xl border border-[#d9b343]/20 flex justify-between items-center mb-2">
                      ...
                      <div className="text-right">
                        <p className="font-bold text-[#7693a6] text-[10px] uppercase tracking-widest">Bill No.</p>
                        <p className="font-bold text-sm text-[#223149]">{paymentModal.billNo}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        <label className="font-bold text-[#7693a6] text-[10px] uppercase tracking-wider flex justify-between items-center">
                          <span>Discount Captured (If any)</span>
                          {paymentModal.discountAvailable > 0 && <span className="text-[#d9b343] font-bold text-[9px]">Avail: ฿{paymentModal.discountAvailable?.toLocaleString()}</span>}
                        </label>
                        <div className="relative">
                          <span className="absolute left-3 top-2.5 text-[#7693a6] font-bold">฿</span>
                          <input 
                            type="number" step="0.01" 
                            value={paymentForm.discountCaptured} onChange={(e: any) => setPaymentForm({...paymentForm, discountCaptured: e.target.value})} 
                            className="w-full px-3 py-2 pl-8 border border-slate-200 rounded-lg outline-none focus:border-[#496ca8] font-black text-lg text-right text-[#d9b343]" 
                            placeholder="0.00" 
                          />
                        </div>
                      </div>
                    ...
                    <div className="pt-4 flex justify-end gap-3 border-t border-slate-100">
                      ...
                      <button 
                        type="submit" 
                        disabled={!(parseFloat(paymentForm.amount) > 0 || parseFloat(paymentForm.discountCaptured) > 0) || (parseFloat(paymentForm.amount||0) + parseFloat(paymentForm.discountCaptured||0)) > paymentModal.balance}
