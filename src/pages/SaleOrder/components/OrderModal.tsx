  ...
  return (
    <div className="fixed inset-0 bg-[#111f42]/70 backdrop-blur-md z-[100] flex items-center justify-center p-4 no-print">
      <div className="bg-white rounded-[2.5rem] w-full max-w-6xl shadow-2xl flex flex-col h-[90vh] border border-white/20 overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="px-8 py-6 flex justify-between items-center bg-[#111f42] text-white shrink-0">
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-64 bg-slate-50 border-r border-slate-200 flex flex-col p-5 gap-2 shrink-0">
            {[
              ...
              <button 
                ...
                className={`w-full text-left px-5 py-4 rounded-2xl text-xs font-normal uppercase tracking-widest flex items-center gap-4 transition-all ${activeModalTab === t.id ? 'bg-[#111f42] text-white shadow-xl shadow-navy/20 translate-x-1' : 'text-slate-500 hover:bg-white hover:text-[#111f42]'}`}
              >
                <t.icon size={18} /> {t.id}
              </button>
            ))}
          ...
          <div className="flex-1 overflow-y-auto p-10 bg-white kanban-scroll relative">
                ...
                <div className="space-y-6 animate-in fade-in duration-300 pb-20">
                  ...
                  <div className="space-y-4">
                    {orderModal.data.items.map((item, idx) => (
                      <div key={idx} className="bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100 flex flex-wrap gap-4 items-end relative">
              ...
              {activeModalTab === 'Delivery Schedule' && (
                <div className="space-y-8 animate-in fade-in duration-300">
                  ...
                  {orderModal.data.items.map((item, itemIdx) => {
                    if (!item.sku) return (
                      <div key={itemIdx} className="bg-slate-50 p-10 rounded-[2rem] border-2 border-dashed border-slate-200 text-center flex flex-col items-center justify-center gap-3">
                    ...
                    return (
                      <div key={itemIdx} className={`bg-white p-6 rounded-[2rem] border-2 shadow-sm relative overflow-hidden transition-all ${isOver ? 'border-red-200' : isMatch ? 'border-slate-100' : 'border-amber-200'}`}>
        ...
        <div className="px-8 py-6 border-t border-slate-200 bg-slate-50 flex justify-end gap-4 shrink-0 rounded-b-[2.5rem]">
          <button type="button" onClick={() => setOrderModal(null)} className="px-8 py-3 rounded-2xl border-2 border-slate-200 text-slate-500 font-normal text-[10px] uppercase tracking-widest hover:bg-white transition-all">Close</button>
          {orderModal.mode === 'view' ? (
            <button type="button" onClick={() => setOrderModal({...orderModal, mode: 'edit'})} className="px-8 py-3 rounded-2xl bg-[#ab8a3b] text-white font-normal text-[10px] uppercase tracking-widest shadow-lg shadow-gold/20 hover:opacity-90 transition-all">Edit Order</button>
          ) : (
            <button type="button" onClick={saveOrder} className="px-10 py-3 rounded-2xl bg-[#111f42] text-white font-normal text-[10px] uppercase tracking-widest shadow-xl shadow-navy/20 hover:opacity-90 transition-all flex items-center gap-2"><CheckCircle size={14}/> Save Order</button>
          )}
        </div>
      </div>
    </div>
