...
import { X, User, Info, MapPin, CreditCard, Pencil, Save, Settings, Settings2, Phone, Mail, ChevronDown } from 'lucide-react';
...
const SupplierActionModal: React.FC<SupplierActionModalProps> = ({
  modalOpen,
  setModalOpen,
  modalMode,
  setModalMode,
  ...
  return (
    <div className="fixed inset-0 bg-[#111f42]/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 no-print animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border-t-[6px] border-[#ab8a3b]">
        <div className="px-8 py-5 flex justify-between items-center bg-[#111f42] text-white shrink-0 border-b border-slate-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black uppercase tracking-widest">{modalMode==='create' ? 'Register New Supplier' : 'Supplier Profile'}</h2>
              <p className="text-[10px] text-slate-400 font-mono mt-1 font-bold tracking-widest uppercase">ID: {form.supplierID}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setIdConfigModalOpen(true)} className="text-[10px] border border-[#ab8a3b] text-[#ab8a3b] px-3 py-1.5 rounded-lg hover:bg-[#ab8a3b] hover:text-[#111f42] transition-all font-bold uppercase flex items-center gap-1.5">
              <Settings2 size={12} /> Config ID
            </button>
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-[#F9F7F6]">
          <div className="px-8 py-4 bg-white border-b border-slate-100 flex items-center gap-4 shrink-0">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden md:block italic">
              {activeFormTab === 'general' ? 'Basic supplier identity and categorization' : 
               activeFormTab === 'address' ? 'Company location and logistics' :
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <div className="space-y-6 max-w-3xl">
              {activeFormTab === 'general' && (
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-2 gap-6 animate-in fade-in duration-300">
                  ...
                  <div className="col-span-2 space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Supplier ID</label>
                    <input disabled value={form.supplierID || ''} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg outline-none font-bold bg-slate-50 text-slate-500 text-[13px] font-mono" placeholder="Auto Generated..." />
                  </div>
                  <div className="col-span-2 space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Supplier Name</label>
                    <input disabled={modalMode==='view'} value={form.supplierName || ''} onChange={e=>setForm({...form, supplierName: e.target.value})} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:border-[#ab8a3b] outline-none font-bold bg-white disabled:bg-slate-50/50 disabled:text-slate-500 text-[13px]" placeholder="ชื่อบริษัท..." />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Category (หมวดหลัก)</label>
                    <select disabled={modalMode==='view'} value={form.category || 'Supplier'} onChange={e=>{
                      const newCat = e.target.value;
                      setForm({
                        ...form, 
                        category: newCat, 
                        subCategory: masterConfig.subCategories[newCat]?.[0] || '',
                        supplierID: modalMode === 'create' ? generateSupplierID(newCat) : form.supplierID
                      });
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sub Category (หมวดย่อย)</label>
                    <select disabled={modalMode==='view'} value={form.subCategory || ''} onChange={e=>setForm({...form, subCategory: e.target.value})} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg outline-none focus:border-[#ab8a3b] bg-white cursor-pointer font-bold text-[13px] disabled:bg-slate-50/50 disabled:text-slate-500">
                      {masterConfig.subCategories[form.category || 'Supplier']?.map((sc: string) => <option key={sc} value={sc}>{sc}</option>)}
                    </select>
                  </div>
              ...
              {activeFormTab === 'address' && (
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-1 gap-6 animate-in fade-in duration-300">
                  <div className="border-b border-slate-100 pb-2 mb-2">
                    <h4 className="text-[12px] font-black text-[#111f42] uppercase tracking-widest">COMPANY ADDRESS</h4>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1"><MapPin size={12}/> Full Address</label>
                    <textarea disabled={modalMode==='view'} rows={4} value={form.vendorAddress || ''} onChange={e=>setForm({...form, vendorAddress: e.target.value})} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:border-[#ab8a3b] outline-none text-[13px] resize-none bg-white disabled:bg-slate-50/50 disabled:text-slate-500" placeholder="ที่อยู่บริษัท..." />
                  </div>
                </div>
              )}

              {activeFormTab === 'contact' && (
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-300">
                  <div className="col-span-2 border-b border-slate-100 pb-2 mb-2">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-200 flex justify-between items-center px-8 rounded-b-2xl">
          ...
          <div className="flex gap-2">
            {modalMode==='view' ? (
              <button onClick={()=>setModalMode('edit')} className="bg-[#ab8a3b] text-white px-8 py-3 rounded-xl font-black shadow-md uppercase tracking-widest flex items-center gap-2 hover:bg-[#8f712c] transition-all font-mono text-[11px]"><Pencil size={14}/> Edit Profile</button>
            ) : (
              <button onClick={handleSave} className="bg-[#111f42] text-[#ab8a3b] px-10 py-3 rounded-xl font-black shadow-lg shadow-[#111f42]/20 uppercase tracking-widest flex items-center gap-2 hover:brightness-110 transition-all font-mono text-[11px]"><Save size={16}/> Save Changes</button>
            )}
          </div>
        </div>
      </div>
...
export default SupplierActionModal;
