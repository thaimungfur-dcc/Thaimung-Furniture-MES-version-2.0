...
const NcActionModal: React.FC<NcActionModalProps> = ({
  ...
  const isGeneralDisabled = modalMode === 'view' || modalMode === 'vendor';
  const isResponseDisabled = modalMode === 'view';
  const isFollowupDisabled = modalMode === 'view' || modalMode === 'vendor';
    ...
    <div className="fixed inset-0 bg-[#111f42]/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 no-print animate-in zoom-in duration-200">
          ...
          <DraggableWrapper>
                <div className="bg-white rounded-2xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
                  <div className="px-6 py-2.5 flex justify-between items-center bg-[#2C3F70] border-b-4 border-[#E3624A] text-white shrink-0">
                    <div className="flex items-center gap-3">
                      {modalMode === 'vendor' ? <User size={24} className="text-[#E3624A] bg-white/20 p-1 rounded" /> : <FileWarning size={24} className="text-[#E3624A]" />}
                      <h2 className="text-lg font-semibold uppercase tracking-widest">{modalMode === 'vendor' ? 'Vendor Response Form' : 'SCAR Analysis & Management'}</h2>
                    </div>
                    ...
                    {[ 
                      ...
                      return (
                        <button 
                          ...
                          className={`px-5 py-3.5 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest border-r border-slate-200 border-b-2 transition-all ${tabStyle}`}
                        >
                  ...
                  <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-slate-50/30 space-y-5 animate-in fade-in duration-300">
                    {modalTab === 'general' && (
                      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input 
                            ...
                            className="w-full px-3 py-2 border rounded-lg outline-none text-[12px] disabled:bg-slate-50" 
                          />
                        </div>
                        <div className="space-y-1.5">
                    ...
                    {modalTab === 'response' && (
                      <div className={`bg-white p-4 sm:p-5 rounded-2xl border shadow-sm space-y-4 animate-in fade-in ${modalMode === 'vendor' ? 'border-4 border-amber-400 bg-amber-50/30' : 'border-slate-200'}`}>
                        <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-100 text-amber-700 flex items-center gap-3">
                          <MessageSquare size={20} />
                          <p className="text-[11px] font-bold italic uppercase tracking-widest">Supplier Root Cause & Corrective Actions</p>
                        </div>
                        <div className="space-y-2">
                          ...
                          <textarea 
                            ...
                            value={formData.vendorResponse?.rootCause || ''} 
                            onChange={e => setFormData({ ...formData, vendorResponse: { ...formData.vendorResponse, rootCause: e.target.value } })} 
                            rows={3} 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <textarea 
                              disabled={isResponseDisabled} 
                              value={formData.vendorResponse?.corrective || ''} 
                              onChange={e => setFormData({ ...formData, vendorResponse: { ...formData.vendorResponse, corrective: e.target.value } })} 
                              rows={3} 
                              className="w-full px-4 py-3 border rounded-xl outline-none focus:border-amber-400 text-[12px] disabled:bg-slate-50" 
                            />
                          </div>
                          <div className="space-y-2">
                            ...
                            <textarea 
                              disabled={isResponseDisabled} 
                              value={formData.vendorResponse?.preventive || ''} 
                              onChange={e => setFormData({ ...formData, vendorResponse: { ...formData.vendorResponse, preventive: e.target.value } })} 
                              rows={3} 
                              className="w-full px-4 py-3 border rounded-xl outline-none focus:border-amber-400 text-[12px] disabled:bg-slate-50" 
                            />
                          </div>
                    ...
                    {modalTab === 'followup' && (
                      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4 animate-in fade-in">
                        <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 text-blue-700 flex items-center gap-3">
                          <ClipboardCheck size={20} />
                          <p className="text-[11px] font-bold italic uppercase tracking-widest">Internal Follow Up (PC/QA)</p>
                        </div>
                  ...
                  <div className="p-4 sm:p-5 bg-slate-50 border-t flex justify-between items-center shrink-0 px-5">
                    <button 
                    ...
                    {modalMode === 'vendor' ? (
                      <button 
                        ...
                        className="px-6 py-2.5 text-white rounded-lg font-bold shadow-md uppercase tracking-widest flex items-center gap-2 hover:opacity-90 transition-all bg-amber-500 text-[12px]"
                      >
                      ...
                      <button 
                        ...
                        className="px-6 py-2.5 text-white rounded-lg font-bold shadow-md uppercase tracking-widest flex items-center gap-2 hover:opacity-90 transition-all bg-[#111f42] text-[12px] disabled:opacity-50"
                      >
