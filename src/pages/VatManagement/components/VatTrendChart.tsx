    ...
    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col h-[320px]">
      <div className="flex justify-between items-center mb-6">
        <h3 className="font-bold text-[#111f42] flex items-center gap-2 text-lg">
          <TrendingUp size={20}/> Net VAT Payable Trend
        </h3>
        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Year 2026</p>
      </div>
      
      <div className="flex-1 relative w-full mt-2">
        <svg viewBox="0 0 1000 200" preserveAspectRatio="none" className="w-full h-[200px] overflow-visible">
          <defs>
          ...
          <path 
            d={`${generateLinePath(data, 'netVat', 200, 1000, maxTrendVat)} L 1000 200 L 0 200 Z`} 
            fill="url(#areaGradient)" 
            className="transition-all duration-300"
          />
          <path 
            d={generateLinePath(data, 'netVat', 200, 1000, maxTrendVat)} 
            fill="none" 
            stroke="#111f42" 
            strokeWidth="4" 
        ...
        {hoveredTrend !== null && (
          <div 
            ...
            style={{ 
              ...
              top: `${(1 - (data[hoveredTrend].netVat / maxTrendVat)) * 100}%`,
              transform: 'translate(-50%, -120%)'
            }}
          >
            <p className="font-bold text-slate-300 mb-1">{data[hoveredTrend].month} 2026</p>
            <p className="flex justify-between gap-4"><span>Net VAT:</span> <span className="font-bold text-[#E3624A]">฿{data[hoveredTrend].netVat?.toLocaleString()}</span></p>
