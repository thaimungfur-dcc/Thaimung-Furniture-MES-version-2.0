    ...
    const saveBOM = async () => {
        ...
        try {
            ...
            Swal.fire({
                icon: 'success',
                title: 'BOM Saved',
                text: 'Data has been successfully synchronized.',
            ...
            Swal.fire({
                icon: 'error',
                title: 'Save Failed',
                text: 'Could not write data to sheet. Please check your connection.',
                confirmButtonColor: '#111f42'
            });
        }
    };
    ...
    return (
        <div className="flex flex-col space-y-4 w-full relative flex-1 transition-colors duration-500 text-[12px]">
                ...
                rightContent={
                    <div className="flex items-center gap-3 w-full lg:w-auto justify-end">
                        <div className="flex bg-[#e2e8f0] p-1 rounded-xl border border-slate-200 shadow-inner h-10 overflow-hidden">
                            <button onClick={() => setViewMode('summary')} className={`px-5 py-1.5 text-[10px] font-black uppercase transition-all rounded-lg ${viewMode === 'summary' ? 'bg-[#111f42] text-white shadow-md' : 'text-slate-500 hover:bg-white/50'}`}><LayoutGrid size={14} className="mr-2 inline" /> Cards</button>
                            <button onClick={() => setViewMode('list')} className={`px-5 py-1.5 text-[10px] font-black uppercase transition-all rounded-lg ${viewMode === 'list' ? 'bg-[#111f42] text-white shadow-md' : 'text-slate-500 hover:bg-white/50'}`}><List size={14} className="mr-2 inline" /> SKU List</button>
                        </div>
            ...
            <div className="w-full flex-1 flex flex-col h-full overflow-hidden px-0 z-10 relative gap-4">
                ...
                <div className="flex flex-col overflow-hidden animate-fade-in-up min-h-[600px] flex-1">
                    {/* Toolbar removed since we use actionButtons in table. However, since there is a grid view (summary) mode that uses searchQuery, we need to keep the search bar for the 'summary' view, but hide it if 'list' view. Wait, actually we can just use the table's search if in list mode. */}
                    {viewMode === 'summary' && (
                        <div className="border border-slate-200 mt-2 p-4 flex flex-col lg:flex-row items-center justify-between gap-4 bg-white rounded-2xl shadow-sm">
                            <div className="relative w-full lg:w-96">
                                <Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search catalogue cards..." className="w-full pl-10 pr-4 py-2.5 text-[11px] font-bold rounded-xl border border-slate-200 focus:outline-none focus:border-[#ab8a3b] bg-slate-50 shadow-inner" />
                            </div>
                            <div className="flex gap-2">
                                <button onClick={handleCreateNew} className="py-2.5 px-6 bg-[#111f42] text-white rounded-xl text-[10px] font-black tracking-widest uppercase shadow-lg shadow-blue-900/20 hover:brightness-110 transition-all flex items-center gap-2">
                    ...
                    <div className="flex-1 flex flex-col pt-4 overflow-y-auto master-custom-scrollbar">
                        {viewMode === 'summary' ? (
                                ...
                                {productRows?.map((row, rowIndex) => (
                                    <div key={rowIndex} className="flex flex-col lg:flex-row gap-6 w-full h-auto lg:h-[400px]">
                                        {row?.map((product: any) => (
                                            ...
                                            >
                                                {/* Card Image Section */}
                                                <img src={product.imageLink} alt={product.itemName} className="absolute inset-0 w-full h-1/2 object-cover transition-all duration-700 group-hover:scale-105" />
                                                
                                                {/* Card Info Section */}
                                                <div className="relative z-10 flex flex-col h-1/2 justify-end p-5 bg-white border-t border-slate-100">
                                                    <span className="px-2 py-0.5 bg-slate-100 text-slate-500 text-[8px] font-black tracking-widest uppercase rounded-sm mb-2 inline-block border border-slate-200 w-fit">{product.category}</span>
                                                    <h2 className="text-lg lg:text-xl font-black text-[#111f42] leading-tight mb-1.5 tracking-tight line-clamp-2 font-sans uppercase">{product.itemName}</h2>
                                                    ...
                                                    <div className="flex items-center justify-between border-t border-slate-50">
                                                        ...
                                                        <span className="text-[#10b981] font-black font-mono text-[10px]">ACTIVE</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            <div className="">
                                <ProductCostTable 
                                    ...
                                    onOpenBOM={openBOM}
                                    actionButtons={
                                        <button onClick={handleCreateNew} className="py-2.5 px-6 bg-[#111f42] text-white rounded-xl text-[10px] font-black tracking-widest uppercase shadow-lg shadow-blue-900/20 hover:brightness-110 transition-all flex items-center gap-2 h-10 w-full sm:w-auto mt-4 sm:mt-0">
                                            <Plus size={16} className="text-[#ab8a3b]" strokeWidth={3} /> NEW PRODUCT COST
                                        </button>
                                    }
                                />
