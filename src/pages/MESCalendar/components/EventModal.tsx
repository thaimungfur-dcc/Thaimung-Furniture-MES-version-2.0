        ...
        <div className="bg-white rounded-none w-full max-w-xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-200">
          ...
          <div className="p-5 bg-slate-50 border-t flex justify-end gap-3">
            ...
            {mode !== 'view' ? (
              <button 
                onClick={onSave} 
                className={`px-10 py-2.5 text-white rounded-none font-black shadow-md uppercase tracking-[0.2em] flex items-center gap-2 hover:opacity-90 transition-all ${eventForm.isHoliday ? 'bg-[#E3624A]' : 'bg-[#111f42]'}`}
              >
                <Save size={16}/> {mode === 'create' ? 'บันทึกรายการ' : 'อัปเดตข้อมูล'}
              </button>
            ) : (
              <button onClick={()=>setMode('edit')} className="px-10 py-2.5 bg-[#ab8a3b] text-white rounded-none font-black shadow-md uppercase tracking-[0.2em] flex items-center gap-2 hover:opacity-90 transition-all">
