    ...
    return (
        <div className="modal-overlay no-print" onClick={closeModal}>
            <div className={`bg-white rounded-none w-full border-b-8 border-[#ab8a3b] shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 max-h-[90vh] ${modalType === 'MANUAL' ? 'max-w-4xl' : 'max-w-3xl'}`} onClick={e => e.stopPropagation()}>
                <div className="p-6 bg-[#111f42] text-white flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-6">
                        <div className="w-14 h-14 bg-white/5 border border-white/10 flex items-center justify-center rounded-none shadow-inner text-[#ab8a3b]"><Boxes size={28} /></div>
                        <div>
                ...
                <div className="flex-1 overflow-y-auto master-custom-scrollbar p-10 bg-[#f8fafc] font-sans">
                    <div className="bg-white p-8 border-2 border-slate-100 shadow-sm space-y-8 rounded-none">
                        ...
                        {modalType !== 'MANUAL' ? (
                            <div className="space-y-8">
                                <div className="p-8 bg-white border-2 border-slate-100 flex justify-between items-center rounded-none shadow-sm">
                                    <div>
                                ...
                                <div className="grid grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        ...
                                        <input type="number" value={form.qty} onChange={e => setForm({...form, qty: e.target.value})} className="input-primary text-right font-black text-3xl border-2 border-[#ab8a3b]/50 h-[60px] font-mono rounded-none focus:border-[#ab8a3b] text-[#111f42]" />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <input type="text" value={form.lotNo} onChange={e => setForm({...form, lotNo: e.target.value})} className="input-primary h-[60px] font-mono font-black border-2 border-slate-200 rounded-none focus:border-[#ab8a3b] uppercase tracking-widest text-lg" placeholder="LOT-ID" />
                                    </div>
                                </div>
                                <div className="grid grid-cols-3 gap-8">
                                    <div className="space-y-2">
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] font-mono">Storage Unit</label>
                                        <select value={form.warehouseName} onChange={e => setForm({...form, warehouseName: e.target.value})} className="input-primary font-black border-2 border-slate-200 rounded-none focus:border-[#ab8a3b] h-[50px] uppercase tracking-widest">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input type="text" value={form.location} onChange={e => setForm({...form, location: e.target.value})} className="input-primary font-mono font-black border-2 border-slate-200 rounded-none focus:border-[#ab8a3b] h-[50px] uppercase tracking-widest text-center" placeholder="LOC" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] font-mono">Manufacture Date</label>
                                        <input type="date" value={form.mfgDate} onChange={e => setForm({...form, mfgDate: e.target.value})} className="input-primary font-mono font-black border-2 border-slate-200 rounded-none focus:border-[#ab8a3b] h-[50px]" />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] font-mono">Supplementary Remarks</label>
                                    <input type="text" value={form.remark} onChange={e => setForm({...form, remark: e.target.value})} className="input-primary font-black border-2 border-slate-200 rounded-none focus:border-[#ab8a3b] h-[50px] placeholder:opacity-30" placeholder="Optional logistics notes..." />
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-6">
                                ...
                                {form.manualItems.map((item: any, idx: number) => (
                                    <div key={idx} className="p-6 bg-slate-50 border-2 border-slate-100 grid grid-cols-12 gap-4 relative rounded-none shadow-sm">
                ...
                <div className="p-8 border-t bg-slate-50 flex justify-between gap-6 shrink-0 z-10 rounded-none items-center">
                    <button onClick={closeModal} className="px-8 py-4 text-slate-500 hover:text-[#111f42] text-[12px] font-black hover:bg-slate-200 rounded-none transition-all uppercase tracking-[0.2em] font-mono border border-transparent hover:border-slate-300">Close Form</button>
                    <div className="flex gap-4">
                        <button onClick={() => submitReceive(false)} className="px-10 py-4 bg-white border-2 border-[#111f42] text-[#111f42] hover:bg-slate-50 text-[12px] font-black rounded-none shadow-sm transition-all flex items-center gap-3 uppercase tracking-[0.2em] font-mono">
                            <Save size={18} /> {modalType === 'MANUAL' ? 'Process Batch' : 'Commit Partial'}
                        </button>
                        {modalType !== 'MANUAL' && (
                            <button onClick={() => submitReceive(true)} className="px-10 py-4 bg-[#E3624A] text-white text-[12px] font-black rounded-none shadow-2xl shadow-[#E3624A]/20 transition-all flex items-center gap-3 uppercase tracking-[0.2em] font-mono border-b-4 border-[#c24b33]">
