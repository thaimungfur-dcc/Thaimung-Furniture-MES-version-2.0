        ...
        <div className="modal-overlay z-[80000]" onClick={() => setShowReportPreview(false)}>
            <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-200 max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-y-auto p-4 bg-slate-200 flex justify-center print:p-0 print:bg-white master-custom-scrollbar">
                    <div className="bg-white p-10 shadow-md w-full max-w-[794px] min-h-[1123px] h-max flex flex-col print:shadow-none relative overflow-hidden font-sans">
                        {/* Status Watermark */}
                            ...
                            <table className="w-full text-left text-[11px] border border-slate-200">
                                ...
                                <tbody className="divide-y divide-slate-200">
                                    ...
                                    {[...Array(Math.max(0, 10 - reportData.length))].map((_, i) => (
                                        <tr key={`empty-${i}`}><td className="px-3 py-4 border-r"></td><td className="px-3 py-4 border-r"></td><td className="px-3 py-4 border-r"></td><td className="px-3 py-4 border-r"></td><td className="px-3 py-4"></td></tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="p-4 bg-white border-t flex justify-end gap-3 shrink-0 no-print rounded-b-2xl">
                    ...
                    <button onClick={executeReportPrint} className="px-8 py-2.5 bg-[#ab8a3b] text-white rounded-xl font-bold text-[11px] shadow-md uppercase flex items-center gap-2 hover:bg-[#8f712c] transition-colors tracking-widest font-mono"><Printer size={16}/> Print Report</button>
                </div>
            </div>
        </div>
    );
