      ...
      cell: ({ row }) => {
        ...
        return (
            <div className="flex justify-center items-center gap-1 opacity-60 hover:opacity-100 transition-opacity">
                <button onClick={() => onOpenOrderView(item, 'pending_po')} className="p-1.5 bg-white border border-slate-200 text-slate-400 hover:text-blue-600 hover:border-blue-300 rounded-lg shadow-sm" title="View Detail"><Eye size={14}/></button>
                {item.status !== 'Completed' && (
                    <button onClick={() => onOpenReceiveModal(item, 'PO')} className="p-1.5 bg-white border border-slate-200 text-[#ab8a3b] hover:text-white hover:bg-[#ab8a3b] hover:border-[#ab8a3b] rounded-lg shadow-sm" title="Receive">
                        <Download size={14}/>
                    </button>
                )}
                {item.status === 'In Progress' && (
                    <button onClick={() => onForceClose(item, 'pending_po')} className="p-1.5 bg-white border border-slate-200 text-rose-400 hover:text-white hover:bg-rose-500 hover:border-rose-500 rounded-lg shadow-sm" title="Force Close">
                        <PowerOff size={14}/>
                    </button>
                )}
            </div>
