                ...
                <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
                  <div className="px-6 py-2.5 flex justify-between items-center bg-[#2C3F70] border-b-4 border-[#E3624A] text-white shrink-0">
                    <h2 className="text-lg font-semibold uppercase tracking-widest flex items-center gap-2">
                      <Printer size={20} /> Preview NC Report
                    </h2>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 bg-slate-200 flex justify-center print:p-0 print:bg-white custom-scrollbar">
                    <div className="bg-white p-4 sm:p-5 shadow-md w-full max-w-[794px] min-h-[1123px] h-max flex flex-col print:shadow-none">
                      {/* Header Section */}
                      ...
                      <div className="flex justify-between items-end mb-4">
                        <div>
                          <p className="text-[10px] font-bold text-[#111f42] uppercase tracking-widest">QUALITY ASSURANCE</p>
                          <p className="text-[9px] text-slate-500 font-mono mt-0.5 whitespace-nowrap">FM-QA01-XX, Rev. 00 (2026-Mar-15)</p>
                        </div>
                        <div className="text-center px-4">
                          <h3 className="text-2xl font-black text-[#E3624A] uppercase tracking-widest whitespace-nowrap">NC REPORT</h3>
                          <p className="text-[11px] font-bold mt-0.5 text-[#111f42]">ใบแจ้งให้แก้ไขข้อบกพร่องภายใน</p>
                        </div>
                        <div className="flex justify-end text-[10px] font-mono flex flex-col items-end gap-1">
                          <div className="flex gap-3">
                            <span className="font-bold text-slate-500 w-16 text-right">NC NO:</span> 
                      ...
                      <div className="grid grid-cols-[1.5fr_1fr] gap-3 mb-4">
                        <div className="border border-slate-200 p-4 rounded-lg bg-slate-50/50 flex flex-col justify-center min-h-[100px]">
                          <p className="font-bold border-b border-slate-200 mb-2 pb-1 uppercase text-[#111f42] text-[9px] tracking-widest">Department Information</p>
                          <p className="font-black text-[13px] uppercase text-[#111f42]">{selectedItem.department}</p>
                          <p className="text-slate-600 text-[10px] leading-relaxed mt-1"><b>Process / Item:</b> {selectedItem.item}</p>
                        </div>
                      ...
                      <div className="space-y-3 flex-1">
                        ...
                        <div className="border border-slate-200 rounded-lg p-3 bg-slate-50/20">
                          <h4 className="font-bold text-amber-600 border-b mb-1 pb-1 uppercase text-[10px]">2. Department Response (Root Cause & Action)</h4>
                          <div className="grid grid-cols-1 gap-1 text-[10px]">
                            <p><b>Root Cause:</b> {selectedItem.response?.rootCause || '-'}</p>
                            <p><b>Action:</b> {selectedItem.response?.corrective || '-'}</p>
                            <p><b>Preventive:</b> {selectedItem.response?.preventive || '-'}</p>
                          </div>
                        </div>
                        <div className="border border-slate-200 rounded-lg p-3 bg-white">
                          <h4 className="font-bold text-blue-600 border-b mb-1 pb-1 uppercase text-[10px]">3. QC Follow Up Results</h4>
                      ...
                      <div className="grid grid-cols-3 gap-6 mt-auto py-8 text-center text-[9px] break-inside-avoid">
                        <div className="border border-slate-200 p-3 rounded-lg flex flex-col h-28">
                          <p className="font-bold border-b pb-1 uppercase">Issued By (QC)</p>
                          <div className="flex-1 border-b border-dashed mb-1"></div>
                          <p className="font-bold">{selectedItem.requester}</p>
                        </div>
                        <div className="border border-slate-200 p-3 rounded-lg flex flex-col h-28">
                          <p className="font-bold border-b pb-1 uppercase">Department Accepted</p>
                          <div className="flex-1 border-b border-dashed mb-1"></div>
                          <p className="font-bold">{selectedItem.response?.responder || '-'}</p>
                        </div>
                        <div className="border border-slate-200 p-3 rounded-lg flex flex-col h-28">
                          <p className="font-bold border-b pb-1 uppercase">Followed Up By (QC)</p>
                          <div className="flex-1 border-b border-dashed mb-1"></div>
                  ...
                  <div className="p-4 bg-white border-t flex justify-end gap-3 shrink-0 no-print">
                    ...
                    <button 
                      ...
                      className="px-5 py-2 bg-[#ab8a3b] text-white rounded-lg font-bold text-[12px] shadow-md uppercase flex items-center gap-2 hover:opacity-90 transition-all"
                    >
